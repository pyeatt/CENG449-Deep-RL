...
This file trains the agent using Jonathan Mathew's simulator. The settings for the agent can be found
in my_agent_data.py.
usage: $python my_agent.py
...
def main():
    ...
    while True:
        ...
        # episode until hits bounds
        while abs(data.x) != 5:
            # Do action to get next state
            reward, nextState, nextAction = doAction(socket, action)

            # get values
            q = data.Q[state[0],state[1],state[2],state[3], action]
            nextq = data.Q[nextState[0], nextState[1], nextState[2], nextState[3], nextAction]

            # update
            data.Q[state[0],state[1],state[2],state[3], action] += (data.stepsize * (reward + data.discount * nextq - q))
            state = nextState.copy()
            action = nextAction

        # save values at end of episode
        np.save("training.npy", data.Q)

def initSim(socket):
    #set initial state centered upright
