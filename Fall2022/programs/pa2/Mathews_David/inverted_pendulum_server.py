...
def get_reward(state, u, new_state, threshold = 0.01):
    if new_state[2] > np.pi/2 or new_state[2] < -np.pi/2 or new_state[0] < -3 or new_state[0] > 3:
        return -1
    elif abs(new_state[2]) < threshold and abs(new_state[3]) < 0.1 and abs(new_state[0]) < 0.1:
        return 1
    else:
        return 0
